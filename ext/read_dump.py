# Modified Dump Read Method from ASE, only stable for orthogonal cell with no-scaled positions wrapped.
# txt read method is highly recommended 
import gzip
import struct
...
def lammps_data_to_ase_atoms(
    ...
    unwrapped_positions = get_quantity(["xu", "yu", "zu"], "distance")
    ...
    if quaternions:
        ...
        out_atoms = atomsobj(
            symbols=types, positions=positions-celldisp, pbc=pbc, celldisp=celldisp, cell=cell
        )
        ...
        out_atoms = atomsobj(
            symbols=types,
            positions=unwrapped_positions-celldisp,
            pbc=[0, 0, 0],
            celldisp=celldisp,
            cell=cell)
...
def read_lammps_dump_text(fileobj, index=-1, **kwargs):
    ...
    while len(lines) > n_atoms:
        ...
        if "ITEM: BOX BOUNDS" in line:
            ...
            if len(tilt_items) == 3:
                pbc = ["pp" in d.lower() for d in tilt_items]
            else:
                pbc = (False,) * 3
